...
class TestRemoteCameraRecord:
    ...
    @allure.feature("报警录像>自动延长录像")
    @allure.story("需人工核查日志和录屏")
    @allure.title('验证自动延长录像')
    def test_remote_auto_extended_record(self, device_config):
        # 检查键是否存在，存在则执行当前用例，否则跳过
        remote_items = device_config['ipc']['camera_record']['items']
        BasePage().check_key_in_yaml(remote_items, 'auto_extended_recording')
        ...
        # 验证打开自动延长录像
        RemoteCameraRecord().auto_extended_record()
